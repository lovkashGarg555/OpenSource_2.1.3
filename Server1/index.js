...
const { model, connect, modelforhistroy } = require('./connect'); //  to get my collection and connection function
connect();
const app = express();
...
app.get("/history", async (req, res) => {
    const data=modelforhistroy.find({});
    res.send(data);
})

app.post('/history', (req, resp) => {
    console.log("data of historyn comm from frontend", req.body)
        const data=new modelforhistroy(req.body);

    data.save();
    resp.send({ "message": "data has been stored" });
